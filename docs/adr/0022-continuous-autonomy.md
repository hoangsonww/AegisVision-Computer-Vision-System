...
The intelligence tier ships on-demand agents — the operator opens a
chat, the agent runs to completion, the session ends. Adaptive
autonomy needs agents that run continuously: an optimizer that audits
cost lines every 24 hours, a monitor that reacts to drift signals
within minutes, a reliability agent that closes the loop on canary
rollbacks.
...
2. Doesn't grow a second agent runtime. `agent-service` stays as the
   only runner — `autonomy-orchestrator` just opens sessions on it.
3. Survives signal storms without flooding the LLM gateway or the
   human approver queue.

## Decision

Introduce `autonomy-orchestrator` whose only job is to translate
triggers (cron ticks + bus signals) into agent-service sessions.
...
- **Per-fire session.** Each fire opens a regular `agent-service`
  ...
  the trigger reason. Agent-service handles tools + gates exactly as
...
- **No new bypass of the gate.** Continuous agents inherit the
  refusal-in-code from the on-demand agent path. A jailbroken or buggy
  continuous agent still cannot promote a model without a human
  signature.
...
- **One schedule, one session.** No long-lived "running" agents — they
  start, run, and either complete or block on a gate. Resume reuses the
