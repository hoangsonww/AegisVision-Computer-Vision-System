...
is to retrain on the firehose. The Matroid scar (memory:
`project-aegisvision-key-decisions`) is that the firehose is poisoned by
...
- **Frames never travel on the bus.** Only the URN does (ADR-0008). The
  snapshot is copy-on-promote per Matroid scar; the original frame is
  unaffected.
