// dataset-service: control-plane CRUD for Dataset + DatasetVersion.
//
// HTTP routes live in internal/server; validation in internal/service.
package main
...
import (
	...
	"github.com/aegisvision/services/dataset-service/internal/server"
...
func run() error {
	...
	httpAddr := config.String("AEGIS_HTTP_ADDR", ":8250")
	healthAddr := config.String("AEGIS_HEALTH_ADDR", ":8251")
	dbDriver := config.String("AEGIS_DB_DRIVER", "sqlite")
	dbDSN := config.String("AEGIS_DB_DSN", "file:/tmp/datasets.sqlite?_pragma=journal_mode(WAL)")
