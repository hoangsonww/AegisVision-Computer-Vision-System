...
> **The GPU front door.** Routes inference requests to Triton, applies
> per-tenant model allow-lists, and publishes `inference.*` events for
> downstream consumers (metering, drift, canary, shadow).
...
2. **GPU reservation.** Ask `gpu-scheduler` for a MIG slice. 503 if
   unavailable.
3. **Triton call.** Use Triton's HTTP/2 protocol.
...
| --- | --- | --- |
| `AEGIS_TRITON_URL` | Triton server URL. | yes |
| `AEGIS_GPU_SCHEDULER_URL` | gpu-scheduler gRPC. | yes |
| `AEGIS_NATS_URL` | Bus. | yes |
| `AEGIS_INFER_TIMEOUT` | Per-request deadline. | `2s` |
