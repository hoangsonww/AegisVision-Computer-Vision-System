// media-service: clip/thumbnail/frame metadata + retention + copy-on-promote.
//
// HTTP routes live in internal/server; domain logic in internal/service.
package main
...
import (
	...
	"github.com/aegisvision/services/media-service/internal/server"
...
func run() error {
	...
	httpAddr := config.String("AEGIS_HTTP_ADDR", ":8280")
	healthAddr := config.String("AEGIS_HEALTH_ADDR", ":8281")
	dbDriver := config.String("AEGIS_DB_DRIVER", "sqlite")
	dbDSN := config.String("AEGIS_DB_DSN", "file:/tmp/media.sqlite?_pragma=journal_mode(WAL)")
	objectStoreBase := config.String("AEGIS_MEDIA_OBJECT_STORE", "s3://aegis-media")
	...
	svc := service.New(st, objectStoreBase)
