...
import (
	...
	"github.com/aegisvision/services/cost-accounting/internal/server"
	"github.com/aegisvision/services/cost-accounting/internal/store"

	"github.com/prometheus/client_golang/prometheus"
)

const serviceName = "cost-accounting"
...
func run() error {
	...
	httpAddr := config.String("AEGIS_HTTP_ADDR", ":8330")
	healthAddr := config.String("AEGIS_HEALTH_ADDR", ":8331")
	dbDriver := config.String("AEGIS_DB_DRIVER", "sqlite")
	dbDSN := config.String("AEGIS_DB_DSN", "file:/tmp/costs.sqlite?_pragma=journal_mode(WAL)")
