// training-orchestrator: submit + track ML training jobs. The external
// runner (Argo Workflows / Ray / Kubeflow) is dispatched via a webhook
// configured per-tenant; this service is the catalog + state-machine.
//
// HTTP routes live in internal/server; state-machine in internal/service.
package main
...
import (
	...
	"github.com/aegisvision/services/training-orchestrator/internal/server"
...
const serviceName = "training-orchestrator"
...
func run() error {
	...
	httpAddr := config.String("AEGIS_HTTP_ADDR", ":8270")
	healthAddr := config.String("AEGIS_HEALTH_ADDR", ":8271")
	dbDriver := config.String("AEGIS_DB_DRIVER", "sqlite")
	dbDSN := config.String("AEGIS_DB_DSN", "file:/tmp/training.sqlite?_pragma=journal_mode(WAL)")
