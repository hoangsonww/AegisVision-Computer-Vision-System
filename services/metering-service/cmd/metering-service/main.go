// Command metering-service consumes events/detections/inferences from the
// bus and maintains per-tenant per-day counters. Surfaces /v1/metering for
// billing exports.
//
// HTTP routes live in internal/server; the bus consumer (batched + capped)
// lives in internal/consumer.
package main
...
import (
	...
	"github.com/aegisvision/services/metering-service/internal/server"
	"github.com/aegisvision/services/metering-service/internal/store"

	"github.com/prometheus/client_golang/prometheus"
)

const serviceName = "metering-service"
...
func run() error {
	...
	httpAddr := config.String("AEGIS_HTTP_ADDR", ":8240")
	healthAddr := config.String("AEGIS_HEALTH_ADDR", ":8241")
	natsURL := config.String("AEGIS_NATS_URL", "")
	dbDriver := config.String("AEGIS_DB_DRIVER", "sqlite")
	dbDSN := config.String("AEGIS_DB_DSN", "file:/tmp/metering.sqlite?_pragma=journal_mode(WAL)")
	...
	// Bus consumer (batched, with the canonical billable-event catalog).
	var consumerInst *consumer.Consumer
	if natsURL != "" {
		nc, err := bus.Connect(natsURL)
		if err != nil {
			return err
		}
		consumerInst = consumer.New(logger, nc, st)
	...
	if consumerInst != nil {
		runner.Register("consumer", func(ctx context.Context) error { return consumerInst.Close(ctx) })
	}
